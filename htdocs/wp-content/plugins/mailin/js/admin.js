...
$jQ(document).ready(function(){
    ...
    function change_attribute_tag(attr_type, attr_name, attr_text){
        ...
        switch(attr_type)
        {
            ...
            case 'date':
                $jQ('#sib_field_placeholder').val('dd/mm/yyyy');
            case 'text':
            case 'float':
    ...
    // click redirect url
    $jQ('input[name=is_redirect_url_click]').on('click', function() {
        $jQ('#sib_subscrition_redirect_area').toggle();
    });
    ...
    $jQ('.sib-form-delete').on('click', function() {
   ...
   /* // remove all transients
    $jQ('#sib-remove-cache').on('click',function(){
        var data = {
   ...
   // remove all transients
    ...
    $jQ('.sib-add-to-form').on('click', function(){
        ...
        else if(btn_id == 'sib_add_captcha_btn')
        {
            ...
            if(site_key == '')
            {
                $jQ('#sib-field-form .alert-danger').html('You should input <strong>Site Key</strong>').show(300);
                return false;
            }
            else if(secret_key == '')
            {
                $jQ('#sib-field-form .alert-danger').html('You should input <strong>Secrete Key</strong>').show(300);
                return false;
            }
        }
        else if(btn_id == 'sib_add_termsUrl_btn')
        {
            ...
            if(terms_url == '')
            {
                $jQ('#sib-field-form .alert-danger').html('You should input <strong>Terms URL</strong>').show(300);
                return false;
            }
        }
