...
class Weglot {
    ...
    public function plugin_menu() {
        ...
        if ( isset( $this->request_uri_no_language )
           // && isset( $_POST['settings-updated-nonce'] )
            && sanitize_text_field( wp_unslash( $this->request_uri_no_language ) )
    ...
    public function treatPage( $final ) {
        ...
        if ( ! is_admin() && strpos( $request_uri,'wc-ajax' ) === false && $this->original_l != '' && $this->destination_l != '' ) {
            ...
            ) && strpos( sanitize_text_field( wp_unslash( $_SERVER['HTTP_REFERER'] ) ),'admin' ) === false ) {
    ...
    public function getRequestUri( $home_dir ) {
        if ( $home_dir ) {
            return str_replace( $home_dir,'',sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ) );
        } else {
